...
class DataLoader:
    ...
    def update_skills(self, unique_skills: List):
    ...
    def load_file_into_db(self, job_file_path:str, seeker_file_path:str):
        jobs_df = self.read_csv_in_chunk(job_file_path)
        seekers_df = self.read_csv_in_chunk(seeker_file_path)
    
        for chunk in jobs_df:
            chunk[['id', 'title']].to_sql(JobModel.__tablename__, self.db.engine, if_exists='replace', index=False)
            unique_skills = set(chunk['required_skills'].str.split(',').explode().str.strip())
            self.update_skills(unique_skills)
        ...
        for chunk in seekers_df:
            chunk[['id', 'name']].to_sql(JobSeekerModel.__tablename__, self.db.engine, if_exists='replace', index=False)
    ...
    def generate_result(self, use_sql=True):
        if use_sql:
            sql = '''
                select
...
def main():
    ...
    if input_command == 'init':
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        dl = DataLoader(DATABASE_URL)
        dl.load_file_into_db(sys.argv[2], sys.argv[3])
        df = dl.generate_result()
